	...
	for(int i=0; i<nfaces; i++)
	{
		...
		else
		{
			for(int j=traces[1].size()-1; j >= 0; j--)
			{
				stitchedTraces.push_back(traces[1][j]);
				if(j >= 1)
					stitchedEdgeFace.push_back({ isoEdgesFaces[1][j - 1][0], isoEdgesFaces[1][j - 1][2], isoEdgesFaces[1][j - 1][1] });
			}
		...
		for(int j=0; j<stitchedTraces.size(); j++)
		{
			...
			else
			{
				if (isTraceClose)
				{
					...
					if (v2 == (v1 + 2) % 3)
					{
						extendedF.row(nExtendedFaces) << nExtendedVerts - (stitchedTraces.size() - 1), F(faceId, v1), nExtendedVerts;
						extendedF.row(nExtendedFaces + 1) << F(faceId, v2), F(faceId, v1), nExtendedVerts - (stitchedTraces.size() - 1);
					}
					else
					{
						extendedF.row(nExtendedFaces) << F(faceId, v1), nExtendedVerts - (stitchedTraces.size() - 1), nExtendedVerts;
						extendedF.row(nExtendedFaces + 1) << F(faceId, v1), F(faceId, v2), nExtendedVerts - (stitchedTraces.size() - 1);
					}
