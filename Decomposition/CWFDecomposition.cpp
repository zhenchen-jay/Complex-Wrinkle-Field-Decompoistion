...
namespace ComplexWrinkleField {
    ...
    )
    {
        ...
        tfwShell = WrinkledTensionField::TFWShell(restPos, restMeshCon, curPos, curMeshCon, poissonRatio, thickness, youngsModulus);
    ...
    void CWFDecomposition::initializeAmpOmega(const Eigen::MatrixXd& curPos, MeshConnectivity& curMeshCon, double ampGuess, Eigen::VectorXd& amp, Eigen::MatrixXd& faceOmega)
    {
        ...
        for (int i = 0; i < nfaces; i++)
        {
            ...
            faceOmegaPara.row(i) = std::sqrt(2.0 * faceCompression) * (abar * evec).transpose();	// convert to one-form, assume unit amp
        }
    ...
    {
        CWF upcwf;
        Subdivide(_baseCWF, upcwf, _upsampleTimes, false, &_LoopS0, nullptr, &_upZMat);
    ...
    void CWFDecomposition::buildProjectionMat(const std::unordered_set<int>& clampedVerts, const MeshConnectivity& meshCon, int nverts)
    {
        _nFreeAmp = nverts;
    ...
    void CWFDecomposition::optimizeAmpOmega()
    {
        ...
        auto TFWEnergy = [&](const Eigen::VectorXd& x, Eigen::VectorXd* grad, Eigen::SparseMatrix<double>* hess, bool isProj)
        {
            ...
            if (grad)
                (*grad) = _projTFWMat * (*grad);
            if (hess)
                (*hess) = _projTFWMat * (*hess) * _unprojTFWMat;
