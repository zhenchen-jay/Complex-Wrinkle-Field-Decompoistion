...
void getWrinkledMesh(const MatrixX& V, const Eigen::MatrixXi& F, const VectorX& zvals, MatrixX& wrinkledV,
                     double scaleRatio, bool isTangentCorrection) {
  int nverts = V.rows();
  int nfaces = F.rows();
...
void computeBaryGradient(const Eigen::Vector3d& P0, const Eigen::Vector3d& P1, const Eigen::Vector3d& P2,
                         const Eigen::Vector3d& bary, Eigen::Matrix3d& baryGrad) {
...
VectorX inconsistencyComputation(const Mesh& mesh, const VectorX& edgeW, const VectorX& zval) {
  ...
  for (int i = 0; i < nfaces; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      double theta0 = std::arg(std::complex<double>(zval[v0], zval[v0 + nverts]));
      double theta1 = std::arg(std::complex<double>(zval[v1], zval[v1 + nverts]));

      incons(v0) += std::norm(std::complex<double>(std::cos(theta0 + edgeW(eid)), std::sin(theta0 + edgeW(eid))) -
                              std::complex<double>(std::cos(theta1), std::sin(theta0)));
      incons(v1) += std::norm(std::complex<double>(std::cos(theta0 + edgeW(eid)), std::sin(theta0 + edgeW(eid))) -
                              std::complex<double>(std::cos(theta1), std::sin(theta0)));
    }
  }
  return incons;
...
VectorX normalizeZvals(const VectorX& zvals) {
  ...
  for (int i = 0; i < size; i++) {
    double norm = std::sqrt(zvals[i] * zvals[i] + zvals[i + size] * zvals[i + size]);
    if (norm > 1e-10) {
